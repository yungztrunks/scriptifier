...
* changes made to wrapper done by yungztrunks
...
* 
* bookmarklet created by yungztrunks (yungztrunks.de)
*/
...
$("btn-gen-bookmarklet").onclick = generate_bookmarklet;
...
function set_options_ait() {
	try {
		...
		else localStorage.setItem("uglify-options-disable-ait", 1);
	} catch (e) { }
}
...
function go(throw_on_error) {
	...
	function main() {
		...
		$stats.innerHTML =
			res.code.length +
			" bytes, saved " +
			((1 - res.code.length / input.length) * 100 || 0).toFixed(2) +
			"%";
	}
}
...
function show_error(e, param) {
	...
	if (e instanceof JS_Parse_Error) {
		...
		e =
			...
			"</small>\n\n" +
			(lines[e.line - 2]
				? e.line - 1 + ": " + encodeHTML(lines[e.line - 2]) + "\n"
				: "") +
			e.line +
			": " +
